...
4. Run a broad, history-aware literature search before proposing serious ideas.
   ...
   Reuse existing knowledge before going outside: search quest/global memory first. Only harvest raw sibling quest files when shared cross-quest memory is enabled (`memory.read_visibility_mode=shared_across_quests`) or the operator explicitly asks for a raw cross-quest scan. In that allowed case, enumerate prior quest briefs from the configured DeepScientist home, read same-domain briefs, deep-read materially overlapping `paper/latex/main.tex` files (especially Conclusion + Limitations), and scan `framework_quirks.md` if it exists and is relevant. See playbook §2.1 for the full source-order protocol.
